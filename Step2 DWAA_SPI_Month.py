...
# 合并连续干湿周期的函数
def merge_periods_ic_method(periods, spi_data, tc, rho_c):
    """合并连续的干湿周期"""
    merged_periods = []
    i = 0
    while i < len(periods) - 1:
        ...
        if interval_duration <= tc:
            # 计算间隔期和前一段干湿周期的强度比
            si = sum([1 - abs(spi_data[j]) for j in range(end1 + 1, start2)])  # 间隔期强度
            sf = sum([1 - abs(spi_data[j]) for j in range(start1, end1 + 1)])  # 前一段周期强度
...
# 探测旱涝急转事件的函数
def detect_dwaa_events(spi_data, dry_threshold, wet_threshold, min_duration=10, tc=8, rho_c=0.2):
    dry_periods = []
    wet_periods = []
    dry_start = None
    wet_start = None
    ...
    for i in range(len(spi_data)):
        spi_value = spi_data[i]

        # 干旱事件探测
        if spi_value < dry_threshold:
    ...
    merged_periods = merge_periods_ic_method(all_periods, spi_data, tc, rho_c)

    # 标记每个月是否发生旱涝急转
    months_detected = np.zeros(len(spi_data), dtype=int)
...
# 输入文件夹路径
input_folder_path = r"D:\A-Projects\DWAA Project\2 Optimization of detected DWAA based on the weighting analysis method\SPI Output"
# 输出文件夹路径
output_folder_path = r"D:\A-Projects\DWAA Project\2 Optimization of detected DWAA based on the weighting analysis method\SPI DWAA Results"
...
for file_name in os.listdir(input_folder_path):
    if file_name.endswith(".csv"):
        ...
        # 确保文件包含必要的列
        if 'SPI_21d' in df.columns:
            spi_data = df['SPI_21d'].values
            df['Year'] = pd.to_datetime(df['Date']).dt.year
            df['Month'] = pd.to_datetime(df['Date']).dt.month
            # 探测事件并记录每个月是否发生
            months_detected = detect_dwaa_events(
                spi_data, DRY_THRESHOLD, WET_THRESHOLD, MIN_DURATION, TC, RHO_C
            )
